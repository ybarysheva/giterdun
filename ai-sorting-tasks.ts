...
const TaskSchema = z.object({
  ...
  importance: z.enum(['!!']).nullable(), // Keep for AI input schema for now, but will be null
...
The user now uses a 'flag' to mark importance, not "!!".
...
- All To-Do Tasks:
  {{#each tasks}}
  - Task: { id: {{id}}, title: '{{title}}', effort: {{effort || 'null'}}, isStale: {{isStale}} }
  {{/each}}
